...
@router.get("")
def list_plants(sort: str = "activity", include_dormant: bool = False,
...
@router.get("/{plant_id}")
def get_plant(plant_id: str, user=Depends(require_user), db: Client = Depends(get_db)):
...
def update_plant(plant_id: str, body: PlantUpdate, user=Depends(require_user), db: Client = Depends(get_db)):
    plant = PlantService(db).update(user.id, plant_id, body.model_dump(exclude_none=True))
    return {"ok": True, "data": PlantOut.model_validate(plant)}


@router.delete("/{plant_id}")
def delete_plant(plant_id: str, hard: bool = False, user=Depends(require_user), db: Client = Depends(get_db)):
